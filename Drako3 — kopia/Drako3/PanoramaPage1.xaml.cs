...
namespace Drako3
{
    ...
    public partial class PanoramaPage1 : PhoneApplicationPage
    {
        ...
        {
            await MySerializer<Polygon>.SaveData(board.Hexs[5].Polygon, "current");
            Polygon g = new Polygon();
            g = await MySerializer<Polygon>.LoadData("current");
        ...
        protected async override void OnNavigatedTo(NavigationEventArgs e)
        {
         ...
         //   c.p1 = game.P1;
           ...
           // ClassToSerialize d = new ClassToSerialize();
          //     d= await MySerializer<ClassToSerialize>.LoadData("current");
            ////await MySerializer<Game>.SaveData(game, "current.sav");
            //ClassToSerialize d = await MySerializer<ClassToSerialize>.LoadData("current");
           // Card c = new Card();
            //c.ActionsLeft = 6;
            //game.page = null;
           // await MySerializer<Card>.SaveData(c, "current");
            //await MySerializer<Game>.SaveData(game, "current.sav");
          //  Card d = new Card();
           // d = await MySerializer<Card>.LoadData("current");
        ...
        public  PanoramaPage1()
        {
            ...
            InitializeComponent();
           // worker.InitializeActionsDictionary();
            Card.InitiateActionsDictionary();
            panorama.DefaultItem = panoramaItemBoard;
            P1 = new Player("Zimex", Fraction.Dragon);
           // P1.DrawInitialHand();
            P2 = new Player("Zaremox", Fraction.Dwarf);
            //P2.DrawInitialHand();
            //P1.RenderHand();
            ...
            board = new Board();
            game = new Game(GameType.HOT_SEATS, P1, P2, board);
            //game.Leader = leader;
            //game.Webber = webber;
           ...
           // game.P2 = P2;
            ...
            worker = new Worker(this, DragonGrid,DwarfsGrid,BoardGrid,HandGrid,game,libraryImage,playedCardImage);
            game.ActivePlayer = P1;
           worker.InitializeDamage();
            libraryImage.Source = new System.Windows.Media.Imaging.BitmapImage(
           new Uri(worker.dragonLibrary, UriKind.RelativeOrAbsolute));
            worker.Draw(worker.game.P1, 4);
            worker.Draw(worker.game.P2, 4);
            worker.RenderHand();
            worker.DrawBoard();
            worker.ResetPolygons();
           // worker.InitializeActionsDictionary();
            FUnc();
    ...
    //                }
    //                else
    //                    if (h.Figure == null && selectedFigure != null && game.playedCard != null) //ruch lub ogien, bolt ?
    //                    {
    ...
    //                                    for (int i = 1; i <= 4; i++)
    //                                    {
    //                                        if (Hex.GetHexByAxialCoordinates(hexs, new Point(currentPoint.X, currentPoint.Y  )).Figure != null)
    //                                        {
    //                                            try
    //                                            {
    //                                                boltLines.Remove(Hex.GetHexByAxialCoordinates(hexs, new Point(currentPoint.X, currentPoint.Y + (double)i+1)));
    //                                                boltLines.Remove(Hex.GetHexByAxialCoordinates(hexs, new Point(currentPoint.X, currentPoint.Y + (double)i + 2)));
    //                                                boltLines.Remove(Hex.GetHexByAxialCoordinates(hexs, new Point(currentPoint.X, currentPoint.Y + (double)i + 3)));
    ...
    //                                                try
    //                                                {
    //                                                    boltLines.Remove(Hex.GetHexByAxialCoordinates(hexs, new Point(currentPoint.X+(double)i+1, currentPoint.Y )));
    //                                                    boltLines.Remove(Hex.GetHexByAxialCoordinates(hexs, new Point(currentPoint.X+(double)i+2, currentPoint.Y )));
    //                                                    boltLines.Remove(Hex.GetHexByAxialCoordinates(hexs, new Point(currentPoint.X+(double)i+3, currentPoint.Y )));
    ...
    //                                                    try
    //                                                    {
    //                                                        boltLines.Remove(Hex.GetHexByAxialCoordinates(hexs, new Point(currentPoint.X + (double)i + 1, currentPoint.Y - (double)i + 1)));
    //                                                        boltLines.Remove(Hex.GetHexByAxialCoordinates(hexs, new Point(currentPoint.X + (double)i + 2, currentPoint.Y - (double)i + 2)));
    //                                                        boltLines.Remove(Hex.GetHexByAxialCoordinates(hexs, new Point(currentPoint.X + (double)i + 3, currentPoint.Y - (double)i + 3)));
